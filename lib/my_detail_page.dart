...
import 'package:get/get.dart';
import 'package:getx_project/detail_controller.dart';
...
class _DetailPageState extends State<DetailPage> {
  ...
  // there is 1 variable
  List imgs=[];   // variable name info
  _readData() async{  //private property,  DefaultAssetBundle use for load json fill
    await DefaultAssetBundle.of(context).loadString("json/img.json").then((s){  //once it load on loadString pass the path,   then save it a variable here it's "s"
      setState((){   //update the UI that it's ready and decoding is done
        imgs = json.decode(s);  //decoding the list of json
      });
    });
  }
                  ...
                  Positioned(
                    ...
                    children: [
                      Container(
                        ...
                        onPressed: ()=> fav.favCounter(),  //resource injection
                        color:Colors.white,
                        )
                      ),
                      SizedBox(width: 10,),
